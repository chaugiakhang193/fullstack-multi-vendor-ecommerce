...
const ALLOWLIST = [
  "AUTH_LIMITS", "PRODUCT_LIMITS", "SHOP_LIMITS", "BANK_LIMITS",
  "COUPON_LIMITS", "REVIEW_LIMITS", "CART_LIMITS", "USER_LIMITS",
  "ORDER_LIMITS", "PAYOUT_LIMITS", "SHIPPING_LIMITS", "CATEGORY_LIMITS",
];

const sf = ts.createSourceFile(BE, readFileSync(BE, "utf8"), ts.ScriptTarget.Latest, true);
...
function evalNode(node, path) {
  ...
  if (ts.isObjectLiteralExpression(node)) {
    ...
    for (const p of node.properties) {
      if (!ts.isPropertyAssignment(p)) throw new Error(`[gen-limits] ${path}: chỉ hỗ trợ property assignment`);
      obj[p.name.getText(sf)] = evalNode(p.initializer, `${path}.${p.name.getText(sf)}`);
    }
    return obj;
  }
...
if (missing.length) throw new Error(`[gen-limits] Thiếu trong ${BE}: ${missing.join(", ")}`);
...
const body = ALLOWLIST
  .map((n) => `export const ${n} = ${JSON.stringify(found.get(n), null, 2)} as const;`)
  .join("\n\n");

mkdirSync(dirname(OUT), { recursive: true });
writeFileSync(OUT, banner + body + "\n", "utf8");
console.log(`[gen-limits] Đã sinh ${ALLOWLIST.length} nhóm -> src/constants/limits.generated.ts`);
