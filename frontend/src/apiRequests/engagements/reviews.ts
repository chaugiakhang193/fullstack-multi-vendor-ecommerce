...
const reviewApiRequest = {
  ...
  getProductReviews: (productId: string, query?: { page?: number; limit?: number; rating?: number }) =>
    http.get<ProductReviewsEnvelope>(`/products/${productId}/reviews${buildQuery(query)}`),

  // Seller Endpoints
  sellerGetReviews: (query?: { page?: number; limit?: number; rating?: number }) =>
