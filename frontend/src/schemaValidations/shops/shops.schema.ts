...
export const CreateShopBody = z
  .object({
    name: z
      .string()
      .min(SHOP_LIMITS.NAME_MIN_LENGTH, "Tên gian hàng phải từ 3 ký tự trở lên.")
      .max(SHOP_LIMITS.NAME_MAX_LENGTH, `Tên gian hàng không được vượt quá ${SHOP_LIMITS.NAME_MAX_LENGTH} ký tự.`)
      .trim(),
    description: z
      .string()
      .max(SHOP_LIMITS.DESCRIPTION_MAX_LENGTH, `Mô tả gian hàng không quá ${SHOP_LIMITS.DESCRIPTION_MAX_LENGTH} ký tự.`)
      .trim()
      .optional()
      .or(z.literal("")),
    pickup_address: z
    ...
    account_holder: z
      ...
      .max(BANK_LIMITS.ACCOUNT_HOLDER_MAX_LENGTH, `Tên chủ tài khoản tối đa ${BANK_LIMITS.ACCOUNT_HOLDER_MAX_LENGTH} ký tự.`)
      .transform((val) => val.toUpperCase()), // Tự động convert chữ hoa cho chuẩn tài chính
    account_number: z
      .string()
      .min(BANK_LIMITS.ACCOUNT_NUMBER_MIN_LENGTH, "Số tài khoản ngân hàng không hợp lệ.")
      .max(BANK_LIMITS.ACCOUNT_NUMBER_MAX_LENGTH, `Số tài khoản tối đa ${BANK_LIMITS.ACCOUNT_NUMBER_MAX_LENGTH} ký tự.`)
      .regex(/^[0-9]+$/, "Số tài khoản chỉ được phép chứa các chữ số."),
    bank_name: z
      .string()
      .min(BANK_LIMITS.BANK_NAME_MIN_LENGTH, "Vui lòng nhập hoặc chọn tên ngân hàng.")
      .max(BANK_LIMITS.BANK_NAME_MAX_LENGTH, `Tên ngân hàng tối đa ${BANK_LIMITS.BANK_NAME_MAX_LENGTH} ký tự.`),
    categoryIds: z
      .array(z.string().uuid("ID danh mục không hợp lệ"))
      .min(SHOP_LIMITS.MIN_CATEGORIES, "Gian hàng cần đăng ký ít nhất 1 danh mục kinh doanh."),
  })
  .strict();
...
export const UpdateShopBody = z
  .object({
    name: z
      ...
      .max(SHOP_LIMITS.NAME_MAX_LENGTH, `Tên cửa hàng không vượt quá ${SHOP_LIMITS.NAME_MAX_LENGTH} ký tự.`)
      .optional(),
    description: z
      .string()
      .max(SHOP_LIMITS.DESCRIPTION_MAX_LENGTH, `Mô tả cửa hàng không quá ${SHOP_LIMITS.DESCRIPTION_MAX_LENGTH} ký tự.`)
      .nullable()
      .optional() as any,
    pickup_address: z
      .string()
      .min(SHOP_LIMITS.PICKUP_ADDRESS_MIN_LENGTH, "Địa chỉ lấy hàng phải từ 10 ký tự trở lên.")
      .optional(),
    ...
    gallery: z
      .array(z.any())
      .max(SHOP_LIMITS.GALLERY.MAX_IMAGES, `Tối đa ${SHOP_LIMITS.GALLERY.MAX_IMAGES} ảnh bộ sưu tập.`)
      .optional() as any,
  })
...
export const RejectShopBody = z
  .object({
    reason: z.string().min(SHOP_LIMITS.REJECT_REASON_MIN_LENGTH, `Lý do từ chối phải có ít nhất ${SHOP_LIMITS.REJECT_REASON_MIN_LENGTH} ký tự.`),
  })
...
export const UpdateSettingsBody = z.object({
  name: z
    .string()
    .min(SHOP_LIMITS.NAME_MIN_LENGTH, `Tên gian hàng phải từ ${SHOP_LIMITS.NAME_MIN_LENGTH} ký tự trở lên.`)
    .max(SHOP_LIMITS.NAME_MAX_LENGTH, `Tên gian hàng không được vượt quá ${SHOP_LIMITS.NAME_MAX_LENGTH} ký tự.`)
    .trim(),
  description: z
    .string()
