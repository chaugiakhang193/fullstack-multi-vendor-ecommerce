  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="text-lg font-black text-foreground">Đánh giá sản phẩm</DialogTitle>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 pt-2">
          ...
          <div className="p-3.5 rounded-xl bg-zinc-50 dark:bg-zinc-900 border text-xs space-y-1">
            ...
            {variantName && (
              <div className="text-muted-foreground font-semibold">Phân loại: {variantName}</div>
            )}
          </div>

          {/* Rating Selection */}
          <div className="space-y-2 flex flex-col items-center">
            <span className="text-sm font-bold text-foreground">Chọn mức độ đánh giá:</span>
          ...
          <div className="space-y-2">
            <label htmlFor="comment" className="text-xs font-bold text-foreground uppercase tracking-wider">
