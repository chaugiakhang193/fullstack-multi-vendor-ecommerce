...
export default function SellerSetupPage() {
  ...
  const [existingShop, setExistingShop] = useState<ShopResponseType | null>(null);
  ...
  const onSubmit = async (values: CreateShopBodyType) => {
    ...
    try {
      ...
      toast.success(isRejected ? "Cập nhật và nộp lại yêu cầu thành công!" : "Tạo cửa hàng thành công! Vui lòng chờ admin phê duyệt.");
  ...
  const handleGalleryChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const totalCount = files.length + galleryFiles.length + existingGallery.length;
    if (totalCount > 3) {
      ...
      <Card className="w-full max-w-3xl shadow-xl">
        <CardHeader className="text-center pb-4">
          ...
          <CardDescription className="text-sm mt-1">
            Điền đầy đủ thông tin để tạo cửa hàng của bạn. Admin sẽ phê duyệt trong vòng 1-2 ngày làm việc.
          </CardDescription>
        </CardHeader>
        ...
        <CardContent>
          {isLoadingShop ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3 text-muted-foreground">
              ...
              <p className="text-sm font-semibold">Đang tải thông tin cửa hàng...</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              {isRejected && (
                <div className="bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-900/40 rounded-xl p-4 text-sm text-rose-800 dark:text-rose-300 flex gap-3 mb-6">
                  ...
                  <div>
                    <p className="font-semibold text-rose-950 dark:text-rose-400">Yêu cầu đăng ký đã bị từ chối</p>
                    <p className="mt-1 text-xs opacity-90 text-rose-800 dark:text-rose-300">
                      Yêu cầu đăng ký cửa hàng của bạn đã bị từ chối phê duyệt. Vui lòng kiểm tra hộp thư email của bạn để xem lý do từ chối chi tiết, cập nhật lại các thông tin chưa chính xác bên dưới và nộp lại yêu cầu.
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className="space-y-4">
              ...
              <Field>
                <FieldLabel>Tên cửa hàng <span className="text-rose-500">*</span></FieldLabel>
                <Input
                  {...register("name")}
              ...
              <Field>
                <FieldLabel>Địa chỉ lấy hàng <span className="text-rose-500">*</span></FieldLabel>
                <Input
            ...
            <div className="space-y-4">
              ...
              <Field>
                <FieldLabel>Tên ngân hàng <span className="text-rose-500">*</span></FieldLabel>
                <Input
                  {...register("bank_name")}
              ...
              <Field>
                <FieldLabel>Số tài khoản <span className="text-rose-500">*</span></FieldLabel>
                <Input
                ...
                {errors.bank_account_number && (
                  <FieldError>{errors.bank_account_number.message}</FieldError>
                )}
              </Field>

              <Field>
                <FieldLabel>Tên chủ tài khoản <span className="text-rose-500">*</span></FieldLabel>
                <Input
            ...
            <div className="space-y-4">
              ...
              {errors.categoryIds && (
                <FieldError>{errors.categoryIds.message?.toString()}</FieldError>
              )}
            </div>
            ...
            <div className="space-y-4">
              ...
              {/* Logo */}
              <Field>
                <FieldLabel>Logo cửa hàng <span className="text-rose-500">*</span></FieldLabel>
              ...
              {/* Banner */}
              <Field>
                <FieldLabel>Banner cửa hàng <span className="text-rose-500">*</span></FieldLabel>
              ...
              {/* Gallery */}
              <Field>
                <FieldLabel>Bộ sưu tập ảnh (1-3 ảnh) <span className="text-rose-500">*</span></FieldLabel>
                <div className="space-y-2">
              ...
              >
                {isSubmitting ? (
                  <>
                    ...
                    {isRejected ? "Đang cập nhật & nộp lại..." : "Đang tạo cửa hàng..."}
                  </>
                ) : (
                  <>
                    <Store className="h-4 w-4 mr-2" />
                    {isRejected ? "Cập nhật & Gửi yêu cầu phê duyệt" : "Tạo cửa hàng"}
                  </>
                )}
              </Button>
            </div>
