...
export default function CustomerProfileCouponsPage() {
  ...
  const { data: walletEnvelope, isLoading } = useWalletCoupons({ limit: 100 }, isAuthenticated);
  ...
  const groupedCoupons = useMemo(() => {
    ...
    items.forEach((item) => {
      if (item.is_used) {
        ...
        const endDate = item.coupon.end_date ? new Date(item.coupon.end_date) : null;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 border border-dashed rounded-xl space-y-3">
          ...
          <p className="text-sm text-muted-foreground font-semibold">Không có mã giảm giá nào ở trạng thái này</p>
        </div>
      ) : (
                ...
                <div className="w-24 flex flex-col items-center justify-center p-3 bg-muted/20 shrink-0 select-none">
                  {activeTab === "unused" && (
                    <Link href="/checkout" className="w-full">
                      <Button size="xs" className="w-full bg-zinc-900 hover:bg-zinc-800 text-white dark:bg-zinc-100 dark:hover:bg-zinc-200 dark:text-zinc-900 font-bold text-[10px] py-1.5 rounded-md">
                        Dùng ngay
                      </Button>
                    </Link>
                  )}
