...
import { CreateReviewBodyType, SellerReplyBodyType } from "@/schemaValidations/engagements/reviews.schema";

export const useProductReviews = (productId: string, query?: { page?: number; limit?: number; rating?: number }) => {
  return useQuery({
...
export const useReviewableItems = (query?: { page?: number; limit?: number }) => {
  return useQuery({
...
export const useSellerReviews = (query?: { page?: number; limit?: number; rating?: number }) => {
  return useQuery({
...
export const useSellerReplyReview = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ reviewId, body }: { reviewId: string; body: SellerReplyBodyType }) =>
