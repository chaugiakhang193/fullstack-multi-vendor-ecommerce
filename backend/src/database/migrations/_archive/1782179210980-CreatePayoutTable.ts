...
export class CreatePayoutTable1782179210980 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "payout" ADD "bank_info_snapshot" jsonb NOT NULL`,
    );
    await queryRunner.query(`ALTER TABLE "payout" ADD "resolved_at" TIMESTAMP`);
    await queryRunner.query(
      `ALTER TABLE "payout" ADD "updated_at" TIMESTAMP NOT NULL DEFAULT now()`,
    );
    await queryRunner.query(`ALTER TABLE "payout" ADD "resolved_by" uuid`);
    await queryRunner.query(
      `ALTER TABLE "payout" ALTER COLUMN "amount" SET NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "payout" ALTER COLUMN "commission_fee" SET NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "payout" ALTER COLUMN "commission_fee" SET DEFAULT '0'`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "payout" ADD "status" "public"."payout_status_enum" NOT NULL DEFAULT 'pending'`,
    );
    await queryRunner.query(
      `ALTER TYPE "public"."notification_type_enum" RENAME TO "notification_type_enum_old"`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."notification_type_enum" AS ENUM('order.created', 'order.status_changed', 'review.created', 'review.replied', 'payout.created', 'payout.status_changed')`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."notification_type_enum_old" AS ENUM('order.created', 'order.status_changed', 'review.created', 'review.replied')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "payout" ADD "status" character varying NOT NULL DEFAULT 'pending'`,
    );
    await queryRunner.query(
      `ALTER TABLE "payout" ALTER COLUMN "commission_fee" DROP DEFAULT`,
    );
    await queryRunner.query(
      `ALTER TABLE "payout" ALTER COLUMN "commission_fee" DROP NOT NULL`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "payout" DROP COLUMN "bank_info_snapshot"`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "payout" ADD "period_start" TIMESTAMP`,
    );
  }
}
