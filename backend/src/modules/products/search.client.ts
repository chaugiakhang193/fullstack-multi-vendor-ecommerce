...
export class SearchClient {
  ...
  /**
   * Gọi GET /search lấy candidate đã xếp hạng.
   * - null  = lỗi/timeout/shape sai → caller fallback ILIKE.
   * - []    = 200 nhưng rỗng → kết quả rỗng hợp lệ, KHÔNG fallback.
   */
  async fetchCandidates(
    ...
    try {
      ...
      // Timeout (abort) hoặc lỗi mạng. Nuốt + fallback, không chạm request/user.
      this.logger.warn(
        `[SearchClient] gọi search-service lỗi: ${(err as Error).message} → fallback ILIKE`,
      );
      return null;
    } finally {
