async function runTests() {
  ...
  for (const tc of cases) {
    ...
    try {
      ...
      console.log(`   ✅ Success! Status: ${json.statusCode || response.status}`);
      console.log(`   📊 Meta:`, JSON.stringify(json.data?.meta || json.meta, null, 2));
      
      const items = json.data?.items || json.items || [];
      console.log(`   📦 Items returned: ${items.length}`);
      if (items.length > 0) {
        items.forEach((item: any, i: number) => {
          console.log(`      [Item ${i + 1}] Name: "${item.name}", Price: ${item.price}`);
        });
      }
    } catch (error: any) {
