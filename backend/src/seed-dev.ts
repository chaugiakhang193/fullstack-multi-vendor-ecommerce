...
async function seed() {
  console.log('====== BẮT ĐẦU GIEO HẠT DỮ LIỆU PHÁT TRIỂN (DEV SEEDER - 80 PRODUCTS) ======');
  ...
  try {
    ...
    const emailsToClean = ['Shopprovip1@gmail.com', 'seller_dev@example.com'];
    ...
    if (!fashionCategory) {
      fashionCategory = categoryRepository.create({ name: 'Thời Trang', slug: 'thoi-trang', parent: null });
      await categoryRepository.save(fashionCategory);
    }

    let shirtCategory = await categoryRepository.findOne({ where: { slug: 'ao-thun-nam' } });
    if (!shirtCategory) {
      shirtCategory = categoryRepository.create({ name: 'Áo Thun Nam', slug: 'ao-thun-nam', parent: fashionCategory });
      await categoryRepository.save(shirtCategory);
    }

    let jeanCategory = await categoryRepository.findOne({ where: { slug: 'quan-jean-nam' } });
    if (!jeanCategory) {
      jeanCategory = categoryRepository.create({ name: 'Quần Jean Nam', slug: 'quan-jean-nam', parent: fashionCategory });
      await categoryRepository.save(jeanCategory);
    }

    let shoeCategory = await categoryRepository.findOne({ where: { slug: 'giay-the-thao' } });
    if (!shoeCategory) {
      shoeCategory = categoryRepository.create({ name: 'Giày Thể Thao', slug: 'giay-the-thao', parent: fashionCategory });
    ...
    if (!techCategory) {
      techCategory = categoryRepository.create({ name: 'Điện Tử', slug: 'dien-tu', parent: null });
      await categoryRepository.save(techCategory);
    }

    let phoneCategory = await categoryRepository.findOne({ where: { slug: 'dien-thoai' } });
    if (!phoneCategory) {
      phoneCategory = categoryRepository.create({ name: 'Điện Thoại', slug: 'dien-thoai', parent: techCategory });
      await categoryRepository.save(phoneCategory);
    }

    let laptopCategory = await categoryRepository.findOne({ where: { slug: 'laptop' } });
    if (!laptopCategory) {
      laptopCategory = categoryRepository.create({ name: 'Laptop', slug: 'laptop', parent: techCategory });
      await categoryRepository.save(laptopCategory);
    }

    let accessoryCategory = await categoryRepository.findOne({ where: { slug: 'phu-kien-cong-nghe' } });
    if (!accessoryCategory) {
      accessoryCategory = categoryRepository.create({ name: 'Phụ Kiện Công Nghệ', slug: 'phu-kien-cong-nghe', parent: techCategory });
    ...
    const devShop = shopRepository.create({
      ...
      description: 'Cửa hàng mô phỏng phục vụ quá trình phát triển Fullstack Web. Cung cấp thời trang cao cấp và đồ công nghệ sành điệu.',
    ...
    const productTemplates: Array<{
      ...
      gallery: string[];
      hasVariants: boolean;
      variants?: Array<{ name: string; skuSuffix: string; priceDiff: number; stock: number; images: string[] }>;
    }> = [];
    ...
    for (let i = 1; i <= 15; i++) {
      ...
      productTemplates.push({
        ...
        gallery: [shirtImages[(i + 1) % shirtImages.length], shirtImages[(i + 2) % shirtImages.length]],
        hasVariants: isEven,
        variants: isEven ? [
          { name: 'Màu Đen - Size M', skuSuffix: 'BLK-M', priceDiff: 0, stock: 30, images: [shirtImages[i % shirtImages.length]] },
          { name: 'Màu Đen - Size L', skuSuffix: 'BLK-L', priceDiff: 10000, stock: 40, images: [shirtImages[i % shirtImages.length]] },
          { name: 'Màu Trắng - Size M', skuSuffix: 'WHT-M', priceDiff: 0, stock: 25, images: [shirtImages[(i + 1) % shirtImages.length]] },
          { name: 'Màu Trắng - Size L', skuSuffix: 'WHT-L', priceDiff: 10000, stock: 35, images: [shirtImages[(i + 1) % shirtImages.length]] },
        ] : undefined
      });
    }
    ...
    for (let i = 1; i <= 15; i++) {
      ...
      productTemplates.push({
        ...
        hasVariants: isOdd,
        variants: isOdd ? [
          { name: 'Size 29', skuSuffix: 'SZ29', priceDiff: 0, stock: 15, images: [jeanImages[i % jeanImages.length]] },
          { name: 'Size 30', skuSuffix: 'SZ30', priceDiff: 0, stock: 20, images: [jeanImages[i % jeanImages.length]] },
          { name: 'Size 31', skuSuffix: 'SZ31', priceDiff: 15000, stock: 25, images: [jeanImages[i % jeanImages.length]] },
          { name: 'Size 32', skuSuffix: 'SZ32', priceDiff: 15000, stock: 15, images: [jeanImages[i % jeanImages.length]] },
        ] : undefined
      });
    }
    ...
    for (let i = 1; i <= 15; i++) {
      ...
      productTemplates.push({
        ...
        gallery: [shoeImages[(i + 1) % shoeImages.length], shoeImages[(i + 2) % shoeImages.length]],
        hasVariants: hasVar,
        variants: hasVar ? [
          { name: 'Kích thước 40', skuSuffix: 'SZ40', priceDiff: 0, stock: 10, images: [shoeImages[i % shoeImages.length]] },
          { name: 'Kích thước 41', skuSuffix: 'SZ41', priceDiff: 0, stock: 12, images: [shoeImages[i % shoeImages.length]] },
          { name: 'Kích thước 42', skuSuffix: 'SZ42', priceDiff: 20000, stock: 15, images: [shoeImages[i % shoeImages.length]] },
          { name: 'Kích thước 43', skuSuffix: 'SZ43', priceDiff: 20000, stock: 8, images: [shoeImages[i % shoeImages.length]] },
        ] : undefined
      });
    }
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      productTemplates.push({
        ...
        hasVariants: hasVar,
        variants: hasVar ? [
          { name: '128GB - Đen Nhám', skuSuffix: '128GB-BLK', priceDiff: 0, stock: 10, images: [phoneImages[i % phoneImages.length]] },
          { name: '256GB - Đen Nhám', skuSuffix: '256GB-BLK', priceDiff: 1500000, stock: 15, images: [phoneImages[i % phoneImages.length]] },
          { name: '256GB - Titan Tự Nhiên', skuSuffix: '256GB-TTN', priceDiff: 1800000, stock: 8, images: [phoneImages[(i + 1) % phoneImages.length]] },
        ] : undefined
      });
    }
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      productTemplates.push({
        ...
        hasVariants: hasVar,
        variants: hasVar ? [
          { name: 'Intel Core i5 - 8GB RAM', skuSuffix: 'I5-8G', priceDiff: 0, stock: 5, images: [laptopImages[i % laptopImages.length]] },
          { name: 'Intel Core i7 - 16GB RAM', skuSuffix: 'I7-16G', priceDiff: 3500000, stock: 8, images: [laptopImages[(i + 1) % laptopImages.length]] },
        ] : undefined
      });
    }
    ...
    for (let i = 1; i <= 15; i++) {
      ...
      productTemplates.push({
        ...
        hasVariants: hasVar,
        variants: hasVar ? [
          { name: 'Màu Đen cá tính', skuSuffix: 'BLK', priceDiff: 0, stock: 50, images: [accImages[i % accImages.length]] },
          { name: 'Màu Trắng thanh lịch', skuSuffix: 'WHT', priceDiff: 0, stock: 45, images: [accImages[(i + 1) % accImages.length]] },
        ] : undefined
      });
    }
    ...
    for (const temp of productTemplates) {
      ...
      const slugName = temp.name.toLowerCase()
        .replace(/[^a-z0-9\s]/g, '')
    ...
    console.log(`Đã tạo thành công ${seededCount} sản phẩm và ${variantCount} biến thể kèm ảnh đẹp mắt!`);
  } catch (error) {
